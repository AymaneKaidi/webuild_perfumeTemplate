...
const toLng: Record<LangCode, string> = { EN: 'en', FR: 'fr', AR: 'ar' }
const toCode: Record<string, LangCode> = { en: 'EN', fr: 'FR', ar: 'AR' }

function Badge({ count }: { count: number }) {
  if (count === 0) return null
  return (
    <span className="absolute -end-1.5 -top-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-gold font-sans text-[9px] font-semibold text-charcoal">
      {count > 9 ? '9+' : count}
    </span>
  )
}
...
export default function Navbar() {
  ...
  const activeLang = toCode[i18n.language] ?? 'EN'
  ...
  const handleLangChange = (code: LangCode) => {
    i18n.changeLanguage(toLng[code]) // persisted to localStorage automatically
  }

  return (
    <header
      className={`fixed start-0 end-0 top-0 z-50 transition-all duration-400 ${
        scrolled ? 'border-b border-charcoal-border bg-charcoal-card backdrop-blur-sm' : 'bg-transparent'
      }`}
    >
      <div className="container mx-auto flex h-16 items-center justify-between px-6 md:h-20">
        ...
        <nav className="hidden items-center gap-8 md:flex" aria-label="Primary navigation">
          {navLinks.map((link) => (
            <Link key={link.href} to={link.href} id={`nav-link-${link.href.replace('/', '') || 'home'}`}
        ...
        <div className="flex items-center gap-4">
          ...
          {/* Wishlist */}
          <button id="nav-wishlist-btn" aria-label={t('nav.wishlist')} onClick={openWishlistDrawer}
          ...
          {/* Cart */}
          <button id="nav-cart-btn" aria-label={t('nav.cart')} onClick={openCartDrawer}
            className="relative text-cream-muted transition-colors hover:text-gold">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          ...
          {/* Mobile hamburger */}
          <button id="nav-mobile-menu-btn" aria-label={t('nav.openMenu')}
      ...
      {mobileOpen && (
        <div className="border-t border-charcoal-border bg-charcoal-card px-6 py-6 md:hidden">
          <nav className="flex flex-col gap-5" aria-label="Mobile navigation">
            {navLinks.map((link) => (
              <Link key={link.href} to={link.href} id={`nav-mobile-link-${link.href.replace('/', '') || 'home'}`}
          ...
          <div className="mt-6 flex items-center gap-2 border-t border-charcoal-border pt-5">
            {languages.map((lang, i) => (
              <span key={lang} className="flex items-center">
                <button id={`lang-mobile-${lang.toLowerCase()}`} onClick={() => handleLangChange(lang)}
