  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-400 ${
        scrolled
          ? 'bg-charcoal-card border-b border-charcoal-border backdrop-blur-sm'
          : 'bg-transparent'
      }`}
    >
            ...
            {languages.map((lang, i) => (
              <span key={lang} className="flex items-center">
                ...
                {i < languages.length - 1 && (
                  <span className="mx-1 text-charcoal-border text-xs select-none">|</span>
                )}
              </span>
            ))}
          </div>
        ...
        <div className="border-t border-charcoal-border bg-charcoal-card px-6 py-6 md:hidden">
          ...
          <div className="mt-6 flex items-center gap-2 border-t border-charcoal-border pt-5">
            {languages.map((lang, i) => (
              <span key={lang} className="flex items-center">
                <button
                  id={`lang-mobile-${lang.toLowerCase()}`}
                  onClick={() => setActiveLang(lang)}
                  className={`font-sans text-xs tracking-wider ${
                    activeLang === lang ? 'text-gold' : 'text-cream-muted'
                  }`}
                >
                  {lang}
                </button>
                {i < languages.length - 1 && (
                  <span className="mx-2 text-charcoal-border text-xs select-none">|</span>
                )}
              </span>
            ))}
          </div>
